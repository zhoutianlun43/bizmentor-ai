 ...
 * 辅助女装电商选品。调用 Research Engine（研究）+ Domain（电商画像）+ Memory（历史类似案例）。
...
export function createProductSelectionSkill(deps: SkillDeps): BizSkill {
  return {
    ...
    name: "选品分析助手",
    description: "辅助女装电商选品：市场机会 / 用户需求 / 竞争 / 风险 / 建议动作 / 历史类似案例",
    domain: "ecommerce",
    ...
    async run(_ctx, input): Promise<SkillOutput> {
      ...
      const description = `女装电商选品：${i.productIdea}${i.category ? "，类目 " + i.category : ""}${i.priceRange ? "，价格带 " + i.priceRange : ""}${i.targetUser ? "，目标用户 " + i.targetUser : ""}`;
